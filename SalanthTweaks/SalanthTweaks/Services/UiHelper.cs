...
public unsafe class UiHelper(ILogger<UiHelper> Log) : IDisposable
{
    ...
    public T* GetNodeById<T>(AtkUnitBase* unitBase, uint nodeId, NodeType? type = null) where T : unmanaged =>
        GetNodeById<T>(&unitBase->UldManager, nodeId, type);

    public T* GetNodeById<T>(AtkComponentBase* component, uint nodeId, NodeType? type = null) where T : unmanaged =>
        GetNodeById<T>(&component->UldManager, nodeId, type);
    ...
    public bool TryMakeImageNode(
        uint id, NodeFlags resNodeFlags, uint resNodeDrawFlags, byte wrapMode, ImageNodeFlags imageNodeFlags,
    ...
    public void LinkNodeAfterTargetNode<T>(T* atkNode, AtkComponentBase* parent, AtkResNode* targetNode)
        where T : unmanaged
    {
        var node = (AtkResNode*)atkNode;
        var prev = targetNode->PrevSiblingNode;
